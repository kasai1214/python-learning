...
日本郵便が提供する東京都の郵便番号CSVデータを基に、該当住所の位置を地図上にマップピンとして可視化するPythonシステムです。

実務で求められるデータの抽出・加工・可視化の一連の流れを理解することを目的として作成しました。
・郵便番号から住所や座標を取得するための外部APIを利用
・重複データの排除による処理効率化
・エラー発生時でも処理を継続する例外処理の実装
の3点を行い、流れの理解だけでなく、運用を意識した設計を目指しました。
